...
export const getServerDate = async (
  ...
  try {
    ...
    return estimateServerTime(reverseIndex(samples, 1), reverseIndex(samples, 0));
  } catch (err) {
    throw err;
  }
};
...
const createSample = (delayTime, samplePromise) => {
  return createDelay(delayTime).then(samplePromise);
};
...
const repeatedSample = (delayTime, sampleList, samplePromise) => {
  return (
    createSample(delayTime, samplePromise)
      //store the sample
      .then((sample) => {
...
const estimateServerTime = (sampleBefore, sampleAfter) => {
  ...
  if (!hasCapturedTick(sampleBefore, sampleAfter)) {
    console.error(
      `A tick was not captured in the samples provided. cannot calculate a more accurate server time. falling back to the server-provided date.`
    );
  ...
  const tickWindow =
    sampleBefore.requestDate.getTime() - sampleAfter.responseDate.getTime();
